...
class Camera:
    ...
    def load_config(self, config):
        ...
        if platform.system() == 'Windows':
            self.multiple_monitors = True if GetSystemMetrics(78) > self.MONITOR_LIMIT else False
    ...
    def start(self, cap):
        ...
        self.cap.set(10, 180)  # brightness     min: 0   , max: 255 , increment:1  
        self.cap.set(11, 140)  # contrast       min: 0   , max: 255 , increment:1     
        self.cap.set(12, 255) # saturation     min: 0   , max: 255 , increment:1
        self.cap.set(13, 255)   # hue         
        # self.cap.set(14, 62  )  # gain           min: 0   , max: 127 , increment:1
        self.cap.set(15, -6) # exposure       min: -7  , max: -1  , increment:1
        self.cap.set(17, 4200) # white_balance  min: 4000, max: 7000, increment:1
    ...
    def update_frame_counter(self):
        ...
        self.frame_counter = self.frame_counter + 1 if not self.pause else self.frame_counter
    ...
    def set_alpha(self, image):
        ...
        background[:,:, 1] = 200
        background[:,:, 2] = 200
        added_image = cv.addWeighted(background,0.1,main_frame,0.9, 0)
